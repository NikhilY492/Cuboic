...
const RESTAURANT_NAME = 'Food Guru';
const RESTAURANT_DESC = 'Robot-delivered dining experience';

const OUTLET_ID = 'outlet-01ld9';
const OUTLET_NAME = 'Food Guru Main Outlet';

const TABLE_IDS = [
    'cmmg60r5k0002c2lwbvxfyg2w', // T1
    'cmmg60t4h000dc2lwyt0b507p', // T2
    'cmmg60t4i000oc2lwdzdodk9w', // T3
    'cmmg60t4h000fc2lwzw3cjheb', // T4
    'cmmg60t4h000ic2lwkp94a8z4', // T5
    'cmmg60t4h000ac2lwj8kanmyu', // T6
    'cmmg60t4h0009c2lwhnwxsbr0', // T7
    'cmmg60t4i000lc2lwq4sglhwz', // T8
    'cmmg60t4h0006c2lw33g5vmks', // T9
    'cmmg60t4i000nc2lw2v6tyq8n', // T10
    'cmmg60t4h000gc2lwwnwfzuyq', // T11
    'cmmg60t4g0004c2lwb5466213', // T12
    'cmn0f3wm20006dj2bcnt2ocrh'  // T13
];

function toTitleCase(str: string): string {
    return str.toLowerCase().replace(/(^|\s|\(|\/)([a-z0-9])/g, (m) => m.toUpperCase());
}
...
async function restore() {
    ...
    // 1. Create Restaurant
    console.log(`🏗️ Creating Restaurant: ${RESTAURANT_NAME}...`);
    ...
    // 2. Create Outlet
    console.log(`📍 Creating Outlet: ${OUTLET_NAME}...`);
    ...
    for (let i = 0; i < MENU_DATA.length; i++) {
        ...
        const category = await prisma.category.create({
            data: {
                restaurantId: restaurant.id,
                name: toTitleCase(catData.category),
                display_order: i,
                is_active: true
            }
        });
    ...
    const hashedPassword = await bcrypt.hash('foodguru123', 10);
    
    await prisma.user.upsert({
        where: { user_id: 'fg_owner' },
        update: { restaurantId: restaurant.id, outletId: outlet.id, role: UserRole.Owner },
        create: {
            user_id: 'fg_owner',
    ...
    await prisma.user.upsert({
        where: { user_id: 'fg_staff01' },
        update: { restaurantId: restaurant.id, outletId: outlet.id, role: UserRole.Staff },
        create: {
            user_id: 'fg_staff01',
            name: 'Food Guru Staff 01',
            password_hash: hashedPassword,
            restaurantId: restaurant.id,
            outletId: outlet.id,
            role: UserRole.Staff
...
restore()
    .catch(e => { console.error('❌ Error during restoration:', e); process.exit(1); })
