...
describe('RestaurantsService', () => {
    ...
    describe('findById', () => {
        ...
        it('returns restaurant with is_occupied=false when no active orders', async () => {
            ...
            prisma.restaurant.findUnique.mockResolvedValue(makeRestaurant({ tables: [table] }));
        ...
        it('includes geofence fields in the response', async () => {
            prisma.restaurant.findUnique.mockResolvedValue(
                makeRestaurant({ geofenceEnabled: true, geofenceRadius: 30, latitude: 10.0, longitude: 76.0 }),
            );
            prisma.order.findMany.mockResolvedValue([]);
        ...
        it('queried active order statuses are the correct 5 states', async () => {
            prisma.restaurant.findUnique.mockResolvedValue(makeRestaurant({ tables: [] }));
            prisma.order.findMany.mockResolvedValue([]);
    ...
    describe('findTables', () => {
        ...
        it('auto-creates a Takeaway table when none exists', async () => {
            ...
            const newTakeaway = makeTable({ id: 'takeaway-id', table_number: 'Takeaway' });
            ...
            expect(prisma.table.create).toHaveBeenCalledWith({
                data: { restaurantId: 'rest-1', table_number: 'Takeaway', is_active: true },
            });
    ...
    describe('findAll', () => {
        it('returns all restaurants', async () => {
            const restaurants = [makeRestaurant(), makeRestaurant({ id: 'rest-2', name: 'Second' })];
    ...
    describe('update', () => {
        it('passes geofence config correctly to prisma.update', async () => {
            const updatedRestaurant = makeRestaurant({ geofenceEnabled: true, geofenceRadius: 15 });
            prisma.restaurant.update.mockResolvedValue(updatedRestaurant);

            const patch = { geofenceEnabled: true, geofenceRadius: 15, latitude: 9.9, longitude: 76.3 };
