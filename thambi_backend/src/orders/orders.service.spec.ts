...
describe('OrdersService', () => {
    ...
    let inventoryConsumption: jest.Mocked<Pick<InventoryConsumptionService, 'consumeOrderInventory'>>;
    ...
    beforeEach(async () => {
        ...
        inventoryConsumption = { consumeOrderInventory: jest.fn().mockResolvedValue(undefined) };
        ...
        const module: TestingModule = await Test.createTestingModule({
            providers: [
                ...
                { provide: InventoryConsumptionService, useValue: inventoryConsumption },
    ...
    describe('updateStatus', () => {
        ...
        it('accepts a current version (equal) and updates status', async () => {
            ...
            const result = await service.updateStatus('order-1', { status: 'Confirmed', version: 2 });
            expect(result.status).toBe('Confirmed');
            expect(eventsGateway.emitToRestaurant).toHaveBeenCalledWith(
                'rest-1', 'order:updated', updatedOrder,
            );
        });
        ...
        it('emits order:print_kot when transitioning to Preparing', async () => {
            ...
            await service.updateStatus('order-1', { status: 'Preparing', version: 1 });

            expect(eventsGateway.emitToRestaurant).toHaveBeenCalledWith(
                'rest-1', 'order:print_kot', updatedOrder,
            );
            expect(eventEmitter.emit).toHaveBeenCalledWith('order.preparing', updatedOrder);
        });
        ...
        it('does NOT emit order:print_kot when already in Preparing', async () => {
            ...
            const kotCalls = (eventsGateway.emitToRestaurant as jest.Mock).mock.calls.filter(
                c => c[1] === 'order:print_kot',
            );
    ...
    describe('cancelOrder', () => {
        ...
        it('throws NotFoundException when order does not exist', async () => {
            prisma.order.findUnique.mockResolvedValue(null);
            await expect(service.cancelOrder('ghost-order')).rejects.toThrow(NotFoundException);
        });

        it('staff with Owner role can cancel a Preparing order', async () => {
            const order = makeOrder({ status: 'Preparing', version: 1 });
            prisma.order.findUnique.mockResolvedValue(order);
            usersService.findById.mockResolvedValue({ role: 'Owner', dashboard_config: [] } as any);
            prisma.order.update.mockResolvedValue({ ...order, status: 'Cancelled' });

            const result = await service.cancelOrder('order-1', 'user-owner');
            expect(result.status).toBe('Cancelled');
            expect(auditService.logAction).toHaveBeenCalledWith(
                'rest-1', 'user-owner', 'Cancel Order', expect.any(Object),
            );
        });
        ...
        it('staff without permission cannot cancel', async () => {
            const order = makeOrder({ status: 'Preparing', version: 1 });
            prisma.order.findUnique.mockResolvedValue(order);
            usersService.findById.mockResolvedValue({ role: 'Kitchen', dashboard_config: [] } as any);

            await expect(service.cancelOrder('order-1', 'user-kitchen')).rejects.toThrow(BadRequestException);
        });
    });
    ...
    describe('markItemsDelivered', () => {
        it('sets status to Delivered when all items are delivered', async () => {
            ...
            prisma.order.update.mockImplementation(({ data }) =>
                Promise.resolve({ ...order, ...data, status: data.status ?? order.status }),
            );

            const result = await service.markItemsDelivered('order-1', ['item-1', 'item-2']);
        ...
        it('throws NotFoundException for unknown order', async () => {
            prisma.order.findUnique.mockResolvedValue(null);
            await expect(service.markItemsDelivered('ghost', ['item-1'])).rejects.toThrow(NotFoundException);
        });
    });
    ...
    describe('getOrCreateSession', () => {
        it('returns existing active session and updates lastActivityAt', async () => {
            ...
            prisma.tableSession.update.mockResolvedValue({ ...existingSession, lastActivityAt: new Date() });
        ...
        it('expires old session and creates a new one after 90 min inactivity', async () => {
            ...
            prisma.tableSession.update.mockResolvedValue({ ...oldSession, status: 'Expired' });
    ...
    describe('mergeOrders', () => {
        it('accumulates quantities for duplicate items', async () => {
            const targetOrder = makeOrder({
                items: [{ itemId: 'item-1', name: 'Biryani', unitPrice: 150, quantity: 1 }],
                version: 0,
            });
        ...
        it('throws BadRequestException when no valid source orders found', async () => {
            ...
            await expect(service.mergeOrders('order-1', ['ghost-order'])).rejects.toThrow(BadRequestException);
        });
    });
    ...
    describe('getUnpaidSummary', () => {
        ...
        it('returns correct total and orderIds for session', async () => {
            ...
            const result = await service.getUnpaidSummary('rest-1', undefined, 'sess-abc');
        ...
        it('resolves customer by phone and returns unpaid summary', async () => {
            ...
            const result = await service.getUnpaidSummary('rest-1', undefined, undefined, '+919999999999');
