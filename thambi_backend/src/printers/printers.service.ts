...
export class PrintersService {
  ...
  const printer = await this.findOne(id, restaurantId);
    ...
    if (printer.connectionType !== 'LAN' || !printer.ipAddress) {
      throw new BadRequestException(
        'Only LAN printers with IP address are supported for direct printing right now.',
      );
    }
  ...
  private async printKotCommand(order: any, isReprint: boolean) {
    ...
    if (printers.length === 0) {
      this.logger.warn(
        `No active KOT printers found for restaurant ${order.restaurantId}`,
      );
      return;
    }
    ...
    for (const printer of printers) {
      try {
        ...
        allSuccess = false;
        this.logger.error(
          `Failed to print KOT on printer ${printer.name}: ${e.message}`,
        );
      }
    }
