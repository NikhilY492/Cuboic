...
export function PaymentsScreen() {
    ...
    const handleMarkFeePaid = async (feeId: string) => {
        Alert.alert('Mark as Paid', 'Confirm that you have paid ₹5 to Onomex for this order?', [
        ...
        <View style={[S.screen, { backgroundColor: colors.bg }]}>
            ...
            <FlatList
                ...
                ListHeaderComponent={
                    ...
                    {/* ── Amount Payable to Onomex (Owner only) ── */}
                    {isOwner && feeSummary && !user?.name?.toLowerCase().includes('alnaas') && (
                        <View style={[styles.onomexSection, { backgroundColor: colors.surface, borderColor: colors.amber }]}>
                            <TouchableOpacity style={styles.onomexHeader} onPress={() => setShowFees(!showFees)} activeOpacity={0.7}>
                                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                                    <Feather name="alert-circle" size={16} color={colors.amber} />
                                    <Text style={[styles.onomexTitle, { color: colors.amber }]}>Amount Payable to Onomex</Text>
                                </View>
                                <Feather name={showFees ? "chevron-up" : "chevron-down"} size={20} color={colors.amber} />
                            </TouchableOpacity>
                            <Text style={[styles.onomexSubtitle, { color: colors.textMuted }]}>
                                ₹5 is owed for each order above ₹100
                            </Text>

                            <View style={styles.onomexKpiRow}>
                                <View style={[styles.onomexKpi, { backgroundColor: colors.surface2, borderColor: colors.border }, styles.onomexKpiDue]}>
                                    <Text style={[styles.onomexKpiValue, { color: colors.amber }]}>₹{feeSummary.totalOwed.toFixed(0)}</Text>
                                    <Text style={[styles.onomexKpiLabel, { color: colors.textMuted }]}>Outstanding</Text>
                                    <Text style={[styles.onomexKpiSub, { color: colors.textDim }]}>{feeSummary.unpaidCount} order(s)</Text>
                                </View>
                                <View style={[styles.onomexKpi, { backgroundColor: colors.surface2, borderColor: colors.border }, styles.onomexKpiPaid]}>
                                    <Text style={[styles.onomexKpiValue, { color: colors.green }]}>₹{feeSummary.totalPaid.toFixed(0)}</Text>
                                    <Text style={[styles.onomexKpiLabel, { color: colors.textMuted }]}>Paid so far</Text>
                                    <Text style={[styles.onomexKpiSub, { color: colors.textDim }]}>{fees.length - feeSummary.unpaidCount} order(s)</Text>
                                </View>
                            </View>
...
const styles = StyleSheet.create({
    ...
    onomexHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
    onomexTitle: { fontSize: 15, fontWeight: '800' },
    onomexSubtitle: { fontSize: 12, marginTop: -4 },
    onomexKpiRow: { flexDirection: 'row', gap: 10 },
    onomexKpi: {
        flex: 1, borderRadius: 10, padding: 12, gap: 2,
        borderWidth: 1,
    },
    onomexKpiDue: { borderColor: '#f59e0b33' },
    onomexKpiPaid: { borderColor: '#22c55e33' },
    onomexKpiValue: { fontSize: 22, fontWeight: '800' },
    onomexKpiLabel: { fontSize: 11, fontWeight: '600' },
    onomexKpiSub: { fontSize: 10 },
    feeListLabel: {
