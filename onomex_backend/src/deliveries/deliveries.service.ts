...
export class DeliveriesService {
  ...
  async create(dto: CreateDeliveryDto) {
    ...
    for (const cabId of allCabinets) {
      ...
      if (!cab)
        throw new BadRequestException(`Cabinet ${cabId} not found on robot`);
      if (cab.status === 'Occupied')
        throw new ConflictException(`Cabinet ${cabId} is occupied`);
    }
  ...
  async confirmStop(deliveryId: string, stopIndex: number) {
    ...
    if (stop.status === 'Delivered')
      throw new BadRequestException('Stop already confirmed');
