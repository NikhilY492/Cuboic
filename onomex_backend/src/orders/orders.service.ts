...
export class OrdersService {
  ...
  async updateStatus(id: string, dto: UpdateOrderStatusDto) {
    ...
    if (existing.status !== 'Cancelled' && dto.status === 'Cancelled') {
      ...
      if (existing.outletId && oldItems.length > 0) {
        try {
          ...
          this.logger.warn(`Failed to refund inventory for cancelled order ${id}: ${e.message}`);
        }
      }
    }
  ...
  async cancelOrder(id: string, userId?: string, incomingVersion?: number) {
    ...
    const oldItems = Array.isArray(existing.items)
      ? (existing.items as Array<{ itemId: string; quantity: number }>)
      : [];
    if (existing.outletId && oldItems.length > 0 && existing.status !== 'Cancelled') {
      try {
        await this.inventoryService.refundForOrder(
          existing.outletId,
          id,
  ...
  async mergeOrders(
    ...
    // Inventory reconciliation for merge
    // 1. Refund targetOrder's existing items
    if (targetOrder.outletId && allTargetItems.length > 0 && targetOrder.status !== 'Cancelled') {
      try {
        await this.inventoryService.refundForOrder(
          targetOrder.outletId,
          targetOrderId,
          allTargetItems.map((i) => ({ itemId: i.itemId, quantity: i.quantity })),
        );
      } catch (err) {}
    }
    
    // 2. Refund sourceOrder's existing items
    for (const sOrder of sourceOrders) {
      const sItems = Array.isArray(sOrder.items) ? (sOrder.items as OrderItem[]) : [];
      if (sOrder.outletId && sItems.length > 0 && sOrder.status !== 'Cancelled') {
        try {
          await this.inventoryService.refundForOrder(
            sOrder.outletId,
            sOrder.id,
            sItems.map((i) => ({ itemId: i.itemId, quantity: i.quantity })),
          );
        } catch (err) {}
      }
    }

    // 3. Deduct targetOrder's merged items
    if (targetOrder.outletId && mergedItems.length > 0) {
      try {
        await this.inventoryService.deductForOrder(
          targetOrder.outletId,
          targetOrderId,
          mergedItems.map((i) => ({ itemId: i.itemId, quantity: i.quantity })),
        );
      } catch (err) {}
    }
