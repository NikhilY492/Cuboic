...
export class InventoryService {
  ...
  async deductForOrder(
    ...
    // Build deduction map: inventoryItemName → total quantity needed
    ...
    for (const orderItem of items) {
      ...
      for (const ing of recipe.ingredients) {
        ...
        const name = ing.inventoryItem.name;
        const needed = ing.quantity * orderItem.quantity;
        const existing = deductions.get(name)?.needed ?? 0;
        deductions.set(name, {
    ...
    const inventoryItems = await this.prisma.inventoryItem.findMany({
      where: { name: { in: Array.from(deductions.keys()) }, outletId },
    });

    const inventoryItemMap = new Map(inventoryItems.map((i) => [i.name, i]));

    for (const [name, { needed }] of deductions.entries()) {
      const item = inventoryItemMap.get(name);
      if (!item) {
        throw new BadRequestException(`Inventory item "${name}" not found in outlet ${outletId}`);
      }
    ...
    await this.prisma.$transaction(
      Array.from(deductions.entries()).flatMap(([name, { needed }]) => {
        const item = inventoryItemMap.get(name)!;
        return [
          this.prisma.inventoryItem.update({
            where: { id: item.id },
  ...
  async refundForOrder(
    ...
    // Build refund map: inventoryItemName → total quantity to refund
    ...
    for (const orderItem of items) {
      ...
      for (const ing of recipe.ingredients) {
        ...
        const name = ing.inventoryItem.name;
        const needed = ing.quantity * orderItem.quantity;
        const existing = refunds.get(name)?.needed ?? 0;
        refunds.set(name, {
          needed: existing + needed,
          outletId,
        });
    ...
    const inventoryItems = await this.prisma.inventoryItem.findMany({
      where: { name: { in: Array.from(refunds.keys()) }, outletId },
    });
    ...
    await this.prisma.$transaction(
      Array.from(refunds.entries()).flatMap(([name, { needed }]) => {
        const item = inventoryItemMap.get(name);
        if (!item) return [];
        return [
          this.prisma.inventoryItem.update({
  ...
  async checkAvailability(
    ...
    for (const orderItem of items) {
      ...
      for (const ing of recipe.ingredients) {
        ...
        const name = ing.inventoryItem.name;
        const needed = ing.quantity * orderItem.quantity;
        neededMap.set(name, (neededMap.get(name) || 0) + needed);
      }
    }

    const inventoryItems = await this.prisma.inventoryItem.findMany({
      where: { name: { in: Array.from(neededMap.keys()) }, outletId },
    });
    const inventoryItemMap = new Map(inventoryItems.map((i) => [i.name, i]));
    ...
    for (const [name, needed] of neededMap.entries()) {
      ...
      const avail = item.currentStock - item.reservedStock;
      if (avail < needed) {
