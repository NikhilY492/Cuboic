...
describe('getSessionId', () => {
    ...
    it('persists session data to localStorage', () => {
        getSessionId();
        const raw = localStorage.getItem('onomex_session_data');
    ...
    it('creates a new session when the stored session is expired', () => {
        ...
        localStorage.setItem('onomex_session_data', JSON.stringify(expiredData));
    ...
    it('extends the expiresAt on each call for a valid session', () => {
        getSessionId();
        const rawBefore = JSON.parse(localStorage.getItem('onomex_session_data')!);
        const expiresBefore = rawBefore.expiresAt;

        // Advance time slightly
        vi.setSystemTime(Date.now() + 5000);
        getSessionId();
        const rawAfter = JSON.parse(localStorage.getItem('onomex_session_data')!);
...
describe('resetSession', () => {
    it('clears the session from localStorage', () => {
        ...
        resetSession();
        expect(localStorage.getItem('onomex_session_data')).toBeNull();
    });

    it('also clears the old legacy key if present', () => {
        localStorage.setItem('onomex_session_id', 'legacy-id');
        resetSession();
        expect(localStorage.getItem('onomex_session_id')).toBeNull();
    });
});
